...
mod sighash_tests {
    ...
    fn sighash_inputs_output_single() {
        ...
        let sighash_buffer = tx.sighash_preimage(sighash, 0, &signing_script, 0, None).unwrap();
    ...
    fn sighash_none_anyonecanpay_no_fork_id() {
        ...
        let sighash_buffer = tx.sighash_preimage(sighash.try_into().unwrap(), 0, &signing_script, 0, None).unwrap();
        assert_eq!(
    ...
    fn sighash_none_anyonecanpay_fork_id() {
        ...
        let _sighash_buffer_all = tx.sighash_preimage(SigHash::InputsOutputs, 0, &signing_script, 1337, None).unwrap();

        let sighash_buffer = tx.sighash_preimage(SigHash::Input, 0, &signing_script, 1337, None).unwrap();
